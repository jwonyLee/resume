# 이지원

- 특정 분야, 플랫폼에 얽매여서 개발하기보다는 관심 있고 흥미 있는 분야라면 다양하게 해보고 싶습니다. 현재는 모바일 개발(iOS, Swift)에 관심을 갖고 공부 중입니다.
- 학습하고, 학습한 내용을 글로 정리하는 걸 좋아합니다. 

## 🔗 Contacts
- Email: jiwon.swdev@gmail.com
- Github: https://github.com/jwonyLee
- Blog: https://jwonylee.tistory.com
- LinkedIn: https://linkedin.com/in/jwonylee

## 💻 프로젝트

### BeepBeep `진행중`
- 🎤 Voice Recorder App to prepare for Interviews and Presentations.
- `iOS`, `Swift`, `Core Data`, `RxSwift`, `RxCocoa`, `SnapKit`, `Then`, `SwiftLint`, `Git`, `Github`
- 기간: 2021.05.03 ~ 2021.07.31
- 저장소: https://github.com/jwonyLee/BeepBeep

### Hazzle-iOS (팀 프로젝트, 3인)
- 습관 관리 iOS 어플리케이션
- 습관 작성 화면, 기능 구현, Realm 객체 설계
- `iOS`, `Swift`, `Realm`, `SnapKit`, `Then`, `SwiftLint`, `Git`, `PuzzleMaker`, `Github`
- 기간: 2020.10 ~ 2020.11
- 저장소: https://github.com/ios-study-boost/Hazzle-iOS
- 관련글: https://jwonylee.tistory.com/66

### 교통대 알리미
- 한국교통대학교 공지사항 새 글 알림 텔레그램 봇
- 게시판 크롤링 후 데이터 저장, 텔레그램 채널로 메시지 전송
- `Python` `PostgreSQL` `Git` `Github`
- 기간: 2020.02
- 저장소: [https://github.com/jwonyLee/knut-bot](https://github.com/jwonyLee/knut-bot)
- 관련글: [https://jwonylee.tistory.com/13?category=860680](https://jwonylee.tistory.com/13?category=860680)

### 5차년도 지역선도대학 육성사업 지역산업체연학 프로젝트 참여
- 공연전시 정보 조회 및 리뷰 작성 어플리케이션
- 공연전시 정보 조회, 한줄평 작성 기능 구현
- `Android`, `Java`, `xml`, `JSON`, `php`, `MySQL`
- 기간: 2018.06 ~ 2018.11

### 모바일 프로그래밍 수업 팀 프로젝트 (2인)
- 파이어베이스 기반의 안드로이트 채팅 어플리케이션
- 회원가입 및 소셜 로그인 구현, 유저 목록 구현
- `Android`, `Java`, `xml`, `JSON`, `Firebase`, `Git`, `Github`
- 기간: 2018.03 ~ 2018.06
- 저장소: [https://github.com/jwonyLee/CommonChat](https://github.com/jwonyLee/CommonChat)

## 💼 활동
### 한빛미디어 도서 서평단 <나는 리뷰어다 2021>
- 기간: 2021.02 ~
- [팀 개발을 위한 Git, GitHub 시작하기](https://jwonylee.tistory.com/88)
- [이것이 안드로이드다 with 코틀린(개정판)](https://jwonylee.tistory.com/89)

### 부스트캠프 2020 챌린지 - 모바일iOS(Swift)
- 기간: 2020.07 ~ 2020.08
- 회고: https://jwonylee.tistory.com/51

### 부스트코스 에이스 1기 - 안드로이드
- edwith - [부스트코스] 안드로이드 프로그래밍
- 미이수
- 기간: 2018.05 ~ 2018.09

### 데이터베이스 동아리(학과 동아리)
- 2018학년도 동아리 회장
- 4차년도, 5차년도 지역선도대학 육성사업 프로젝트 참여
- 기간: 2017 ~ 2018

## 🌱 그 외

### Road-of-CODEr/stupid-week-2021
- 매 주 최소 3개의 목표를 설정하고 지키는 스터디
- 기간: 2021.01 ~
- 저장소: https://github.com/Road-of-CODEr/stupid-week-2021
- [Pull Requests](https://github.com/Road-of-CODEr/stupid-week-2021/pulls?q=is%3Apr+is%3Aclosed+assignee%3AjwonyLee)

### ios-study-boost/clean-code
- `clean code` 책 스터디
- 저장소: https://github.com/ios-study-boost/clean-code
